...
`;function Z(){return(0,R.join)((0,N.tmpdir)(),"brightness-control-ddc.ps1")}function z(){let t=Z();return(0,A.writeFileSync)(t,Y,"utf-8"),t}async function K(t,e=0){let n=z(),r;try{r=(await U(`powershell -ExecutionPolicy Bypass -NoProfile -NonInteractive -File "${n}" -Action ${t} -Value ${e}`,{timeout:15e3,encoding:"utf8"})).stdout}catch(o){let u=o,l=(u.stdout||"").trim().replace(/^\uFEFF/,"");if(l)try{let c=JSON.parse(l);if(c.error)throw new Error(c.error);return c}catch{}let h=u.stderr||u.message||String(o);throw new Error(`Brightness script failed: ${h}`)}let s=r.trim().replace(/^\uFEFF/,""),i=JSON.parse(s);if(i.error)throw new Error(i.error);return i}async function E(t){return(await K("set",t)).monitors}var Q=(0,L.platform)()==="win32";async function O(t){if(Q)try{let i=await E(t);if(i.length===0)return await(0,m.showToast)({style:m.Toast.Style.Failure,title:"No Brightness-Capable Monitors Found",message:"No WMI or DDC/CI monitors detected"}),null;let o=i.find(u=>u.setResult===!0)||i[0];return{displayName:o.description||void 0,previousBrightness:o.brightness,brightness:o.newBrightness}}catch(i){return await(0,m.showToast)({style:m.Toast.Style.Failure,title:"Failed to Set Brightness",message:i instanceof Error?i.message:"An error occurred"}),null}if(!await P())return null;let e=await x();if(e.length===0)return await(0,m.showToast)({style:m.Toast.Style.Failure,title:"No Displays Found",message:"Make sure Lunar is running and displays are connected"}),null;let n=await S(),r=e.find(i=>i.serial===n);r||(r=e.find(i=>i.main)||e[0]);let s=await b(r.serial);return await B(r.serial,t,r.adaptive),{displayName:r.name,previousBrightness:s??void 0,brightness:t}}async function v(){try{let t=await O(100);if(!t)return;t.displayName&&t.brightness!=null?await(0,p.showHUD)(`\u{1F680} ${t.displayName}: ${t.brightness}%`):t.displayName?await(0,p.showHUD)(`\u{1F680} ${t.displayName}: Brightness to the max!`):await(0,p.showHUD)("\u{1F680} Brightness to the max!")}catch(t){console.error("Failed to set max brightness:",t),await(0,p.showToast)({style:p.Toast.Style.Failure,title:"Failed to Set Brightness",message:t instanceof Error?t.message:"An error occurred"})}}
